...
public class App {
	...
	public App(String traccia) {
		...
		espressioni = new ArrayList<Espressione>();
	}
	...
	public void esegui() {
		if(valutaEspressione(traccia)) {
			...
			for(String i : operatori) {
				switch(i) {
				case "+":
					espressioni.add(new EspressioneSomma(espressioniTerminali.get(0), espressioniTerminali.get(1)));
					break;
				case "-":
					espressioni.add(new EspressioneSottrai(espressioniTerminali.get(0), espressioniTerminali.get(1)));
					break;
				case "*":
					espressioni.add(new EspressioneMoltiplica(espressioniTerminali.get(0), espressioniTerminali.get(1)));
					break;
				case "/":
					espressioni.add(new EspressioneDividi(espressioniTerminali.get(0), espressioniTerminali.get(1)));
					break;
				case "^":
					espressioni.add(new EspressionePotenza(espressioniTerminali.get(0), espressioniTerminali.get(1)));
					break;
				default:
					return;
				}
				espressioniTerminali.add(2, new EspressioneTerminale(espressioni.get(espressioni.size() - 1).interpreta().toString()));
